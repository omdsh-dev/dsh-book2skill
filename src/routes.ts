...
import type { GateAnswer, InstallTarget } from './types.ts'
...
export function registerBook2SkillRoutes(service: Book2SkillService): () => void {
...
async function handle(service: Book2SkillService, req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (path === '/book2skill/jobs' && method === 'POST') {
    ...
    const input = (body ?? {}) as { bookPath?: string; title?: string }
  ...
  if (jobMatch !== null) {
    ...
    if (action === '/gate1/answer' && method === 'POST') {
      const body = (await readBody(req)) as { answers?: GateAnswer[] } | undefined
    ...
    if (action === '/draft' && method === 'POST') {
      const body = (await readBody(req)) as { draft?: string } | undefined
    ...
    if (action === '/gate2/decision' && method === 'POST') {
      const body = (await readBody(req)) as { verdict?: 'pass' | 'regenerate'; draft?: string } | undefined
    ...
    if (action === '/gate3/targets' && method === 'POST') {
      const body = (await readBody(req)) as { targets?: InstallTarget[] } | undefined
  ...
  if (path === '/book2skill/zlib/search' && method === 'POST') {
    const body = (await readBody(req)) as { query?: string } | undefined
  ...
  if (path === '/book2skill/zlib/download' && method === 'POST') {
    const body = (await readBody(req)) as { jobId?: string; downloadPath?: string } | undefined
