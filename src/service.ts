...
import { book2skillDomain } from './domain.ts'
import {
  Book2SkillError, bookKeyOf, chaptersDir, ensureWorkDir, failJob, jobHandle, makeJobId, newJob, stageOfStatus, workDir,
  type DomainHandleLike, type JobHandle,
} from './jobs.ts'
import { bookPathOf, parseBook, readChapter } from './parse.ts'
import { downloadZlib, searchZlib } from './zlib.ts'
import { installSkill } from './install.ts'
import type {
  Book2SkillJob, GateAnswer, GateQuestion, InstallTarget, JobListRow, SelfCheckItem, StageNote,
} from './types.ts'
import { INSTALL_TARGET_LABELS } from './types.ts'
...
export class Book2SkillService {
  ...
  private domainPromise: Promise<JobHandle> | undefined
  ...
  async getJob(jobId: string): Promise<Book2SkillJob> {
    const handle = await this.domain()
    const job = handle.get(jobId)
    if (job === undefined) throw new Book2SkillError(`未知任务 ${jobId}`)
    return job
  }

  /** Create a job from a local path, a z-lib download URL, or a bare title. */
  async startJob(input: StartInput): Promise<Book2SkillJob> {
    const handle = await this.domain()
    const id = makeJobId()
    ensureWorkDir(id)
    const book: Book2SkillJob['book'] = { kind: 'local', title: input.title }
    if (input.bookPath !== undefined && input.bookPath.trim() !== '') {
      if (!existsSync(input.bookPath)) throw new Book2SkillError(`书籍文件不存在：${input.bookPath}`)
      book.path = input.bookPath
      if (input.title === undefined) book.title = basename(input.bookPath).replace(/\.(epub|pdf)$/i, '')
      ...
      if (!dl.ok) return failJob(handle, id, new Error(dl.error ?? '下载失败'))
      book.path = dl.path
      if (input.title === undefined && dl.path !== undefined) book.title = basename(dl.path).replace(/\.(epub|pdf)$/i, '')
  ...
  async parse(jobId: string, signal?: AbortSignal): Promise<Book2SkillJob> {
    const handle = await this.domain()
    const job = handle.get(jobId)
    if (job === undefined) throw new Book2SkillError(`未知任务 ${jobId}`)
    if (job.status === 'cancelled') throw new Book2SkillError('任务已取消，无法解析')
    try {
      await parseBook(this.ctx, handle, job, signal)
    } catch (error) {
      const current = handle.get(jobId)
      if (current !== undefined && current.status !== 'cancelled') await failJob(handle, jobId, error)
      throw error
    }
  ...
  async addNote(jobId: string, input: NoteInput): Promise<Book2SkillJob> {
    const handle = await this.domain()
    ...
    return handle.update(jobId, j => {
      if (note.kind === 'draft') {
        ...
        j.notes.push(note)
        if (advanceTo !== undefined) j.status = advanceTo
      }
  ...
  async setQuestions(jobId: string, questions: GateQuestion[]): Promise<Book2SkillJob> {
  ...
  async answerGate1(jobId: string, answers: GateAnswer[]): Promise<Book2SkillJob> {
    const handle = await this.domain()
    return handle.update(jobId, j => {
      if (j.gate1.status !== 'open') throw new Book2SkillError('门控1当前不在等待状态')
  ...
  async addSelfCheck(jobId: string, item: SelfCheckInput): Promise<Book2SkillJob> {
    const handle = await this.domain()
    return handle.update(jobId, j => {
      const index = j.skill.selfcheck.findIndex(existing => existing.id === item.id)
      const record: SelfCheckItem = { id: item.id, title: item.title, pass: item.pass, note: item.note }
      if (index >= 0) j.skill.selfcheck[index] = record
      else j.skill.selfcheck.push(record)
      j.skill.selfcheck.sort((a, b) => a.id.localeCompare(b.id))
    })
  }

  /** Panel saves an edit to the draft without changing the gate verdict. */
  async saveDraft(jobId: string, draft: string): Promise<Book2SkillJob> {
    const handle = await this.domain()
    return handle.update(jobId, j => {
      j.skill.draft = draft
    })
  ...
  async decideGate2(jobId: string, verdict: 'pass' | 'regenerate', editedDraft?: string): Promise<Book2SkillJob> {
    const handle = await this.domain()
    return handle.update(jobId, j => {
      if (j.gate2.status !== 'open') throw new Book2SkillError('门控2当前不在等待状态')
      if (editedDraft !== undefined && editedDraft.trim() !== '') j.skill.draft = editedDraft
  ...
  async setTargets(jobId: string, targets: InstallTarget[]): Promise<Book2SkillJob> {
    const valid = targets.filter((target): target is InstallTarget =>
      target === 'claude' || target === 'codex' || target === 'kk_skill')
    const handle = await this.domain()
    return handle.update(jobId, j => {
  ...
  async confirmInstall(jobId: string): Promise<Book2SkillJob> {
    const handle = await this.domain()
    return handle.update(jobId, j => {
      if (j.install.targets.length === 0) throw new Book2SkillError('请先勾选至少一个安装目标')
  ...
  async install(jobId: string, input: InstallInput): Promise<Book2SkillJob> {
    const handle = await this.domain()
    const job = handle.get(jobId)
    if (job === undefined) throw new Book2SkillError(`未知任务 ${jobId}`)
    if (job.install.confirmed !== true) {
      throw new Book2SkillError(
        '门控3未通过：安装目标尚未在面板确认。请在面板勾选目标并点击“确认安装”，或先调用 book2skill_stage_note 记录目标。',
      )
    }
    const targets = (input.targets ?? job.install.targets).filter((target): target is InstallTarget =>
      target === 'claude' || target === 'codex' || target === 'kk_skill')
    if (targets.length === 0) throw new Book2SkillError('安装目标为空')
  ...
  async cancel(jobId: string): Promise<Book2SkillJob> {
    const handle = await this.domain()
    return handle.update(jobId, j => {
   ...
   */
  async importOcr(jobId: string, inputDir: string): Promise<Book2SkillJob> {
    const handle = await this.domain()
    const job = handle.get(jobId)
    if (job === undefined) throw new Book2SkillError(`未知任务 ${jobId}`)
    if (!existsSync(inputDir)) throw new Book2SkillError(`导入目录不存在：${inputDir}`)
    const files = readdirSync(inputDir).filter(name => name.endsWith('.md') && name !== 'toc.md').sort()
    if (files.length === 0) throw new Book2SkillError(`导入目录没有 .md 文件：${inputDir}`)
    const outDir = chaptersDir(jobId, bookKeyOf(job))
    mkdirSync(outDir, { recursive: true })
    const chapters: Array<{ file: string; title: string; chars: number }> = []
  ...
  async readChapter(jobId: string, file: string, offset: number, limit: number): Promise<{ file: string; text: string; truncated: boolean }> {
    const job = await this.getJob(jobId)
    const chapter = job.parse.chapters.find(c => c.file === file)
    if (chapter === undefined) throw new Book2SkillError(`未知章节文件：${file}（见 parse.chapters 列表）`)
    const outDir = job.parse.outDir
    if (outDir === undefined) throw new Book2SkillError('解析尚未完成')
  ...
  async zlibSearch(query: string): Promise<ReturnType<typeof searchZlib>> {
    return searchZlib(this.ctx, query)
  }

  async zlibDownload(jobId: string, downloadPath: string): Promise<Book2SkillJob> {
    const handle = await this.domain()
    const job = handle.get(jobId)
    if (job === undefined) throw new Book2SkillError(`未知任务 ${jobId}`)
