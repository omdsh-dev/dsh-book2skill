...
function probeService(ctx: Context): {
  parsePdf(pdfPath: string, options: {
    outputDir: string
...
async function tryPaddleRpc(
  ...
  report: OcrProgressSink,
  signal?: AbortSignal,
): Promise<{ pages: number; files: string[] } | undefined> {
  ...
  const jobId = String((start.value as { jobId?: unknown })?.jobId ?? '')
  if (jobId === '') return undefined

  for (;;) {
    if (signal?.aborted) throw new Error('OCR 已取消')
    const status = await rpcCall(port, 'task/status', { jobId }, signal)
    if (status === undefined || !status.ok) break
    const value = status.value as {
      phase?: string; detail?: string; page?: number; totalPages?: number
    }
    ...
    if (phase === 'queued') {
      ...
      if (commit !== undefined && commit.ok) {
        const out = commit.value as { files?: string[]; pageCount?: number }
...
function rpcCall(port: number, endpoint: string, payload: unknown, signal?: AbortSignal): Promise<WireResult | undefined> {
  return new Promise(resolve => {
    ...
    const req = httpRequest({
      ...
      res.on('end', () => {
        ...
        try {
          const parsed = JSON.parse(Buffer.concat(chunks).toString('utf8')) as { result?: WireResult }
...
async function tryHttpStub(
  ...
  if (submit === undefined || !submit.ok) return undefined
  const taskId = String((submit as { taskId?: unknown }).taskId ?? '')
  if (taskId === '') return undefined

  for (;;) {
    if (signal?.aborted) throw new Error('OCR 已取消')
    const poll = await stubJson(ctx, withQuery(endpoint, { task: taskId }), signal)
    if (poll === undefined || !poll.ok) throw new Error(`OCR HTTP 查询失败：${poll?.error ?? '端点不可达'}`)
    const state = (poll as { state?: string }).state
    if (state === 'queued') {
      report({ state: 'queued', page: Number((poll as { page?: unknown }).page ?? 0), total: totalPages, message: '排队中', queued: true })
    } else if (state === 'running') {
      report({ state: 'running', page: Number((poll as { page?: unknown }).page ?? 0), total: totalPages, message: (poll as { message?: string }).message })
    } else if (state === 'done') {
      report({ state: 'done', page: totalPages, total: totalPages, message: 'OCR 完成' })
      return { pages: totalPages, files: (poll as { files?: string[] }).files ?? [] }
    } else if (state === 'error') {
      throw new Error(`OCR 失败：${(poll as { message?: string }).message ?? '未知错误'}`)
    }
    await sleep(1500, signal)
  }
}

interface StubJsonResult {
  ok: boolean
  taskId?: string
  state?: string
  page?: unknown
...
async function stubJson(ctx: Context, url: string, signal?: AbortSignal): Promise<StubJsonResult | undefined> {
  try {
    const response = await ctx.web.fetch({ url }, signal)
    if (response.statusCode >= 400) return { ok: false, error: `HTTP ${response.statusCode}` }
    return JSON.parse(response.body.content) as StubJsonResult
  } catch {
    return undefined
  }
}
