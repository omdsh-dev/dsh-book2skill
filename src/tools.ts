...
import type { GateAnswer, GateQuestion, InstallTarget, SelfCheckItem } from './types.ts'
import { INSTALL_TARGET_LABELS, NOTE_KINDS, STAGES } from './types.ts'
...
function rec(value: JsonValue): Record<string, unknown> {
  return value as unknown as Record<string, unknown>
}

export function registerBook2SkillTools(service: Book2SkillService): (() => void)[] {
  const register = (definition: ReturnType<typeof defineTool>): () => void =>
    service.ctx.tools.register(definition)

  return [
    register(defineTool({
      name: 'book2skill_start',
      description:
        '创建“书籍转技能”任务：一本本地 EPUB/PDF 或 z-lib 下载链接，产出可安装的 skill。'
      ...
      timeoutMs: 300_000,
      async execute(args) {
        const input = args as { bookPath?: string; downloadUrl?: string; title?: string }
        try {
    ...
    register(defineTool({
      ...
      description:
        '解析任务中的书籍：EPUB 走 parse_epub 逻辑（按章 md + toc.md）；文本型 PDF 走 PyPDF2 按 50 页分块；'
    ...
    register(defineTool({
      ...
      description:
        '读取“书籍转技能”任务完整状态：阶段、解析结果（章节树/OCR 进度）、笔记（浅读摘要/知识地图/草稿/自检）、'
    ...
    register(defineTool({
      ...
      description:
        '记录中间产物并推进任务状态：浅读摘要 / 知识地图 / 深读笔记 / SKILL.md 草稿 / 门控问题 / 自检项。'
      ...
      async execute(args) {
        const input = args as unknown as {
          jobId: string; kind?: string; title: string; text: string
          questions?: GateQuestion[]; selfcheck?: SelfCheckItem
        }
        try {
          ...
          const job = await service.addNote(input.jobId, { kind: input.kind as never, title: input.title, text: input.text })
    ...
    register(defineTool({
      ...
      output: {
        schema: { type: 'json' } as const,
        render: (_args, value) => {
          const v = value as { file?: string; text?: string; truncated?: boolean }
          return [{ type: 'text', text: `## ${v.file ?? '章节'}\n${v.text ?? ''}${v.truncated === true ? '\n…（已截断，可用 offset 续读）' : ''}` }]
        },
      },
      async execute(args) {
        const { jobId, file, offset = 1, limit = 200 } = args as { jobId: string; file: string; offset?: number; limit?: number }
        try {
    ...
    register(defineTool({
      ...
      timeoutMs: 600_000,
      async execute(args) {
        const { jobId, downloadPath } = args as { jobId: string; downloadPath: string }
        try {
    ...
    register(defineTool({
      ...
      description:
        'OCR 兜底导入：当 book2skill_parse 报告 OCR 后端不可用时，用 paddle_ocr_layout 工具把扫描型 PDF 解析到某目录，'
      ...
      timeoutMs: 120_000,
      async execute(args) {
        const { jobId, inputDir } = args as unknown as { jobId: string; inputDir: string }
        try {
    ...
    register(defineTool({
      ...
      description:
        '把已通过门控2的 SKILL.md 与章节 references 安装到目标目录（~/.claude/skills、~/.codex/skills、'
      ...
      timeoutMs: 300_000,
      async execute(args) {
        const { jobId, name, targets, overwrite } = args as {
          jobId: string; name?: string; targets?: InstallTarget[]; overwrite?: boolean
        }
        try {
...
function startHint(job: { status: string }): string {
...
function renderStart(value: Record<string, unknown>): string {
  const lines = [
...
function renderParse(value: Record<string, unknown>): string {
  if (value.ok !== true) return `解析失败：${value.error ?? '未知错误'}`
...
function renderJob(value: Record<string, unknown>): string {
  if (value.ok !== true) return `任务读取失败：${value.error ?? '未知错误'}`
  const lines: string[] = [
    `jobId：${value.jobId ?? ''}`,
    `状态：${value.status ?? ''}（${stageLabel(value.stage as number)}）`,
  ]
  const book = value.book as Record<string, unknown> | undefined
  if (book !== undefined) {
    lines.push(`书籍：${book.title ?? book.path ?? book.url ?? '未设置'}（${book.format ?? '未解析'}）`)
  }
  const ocr = value.ocr as { state?: string; page?: number; total?: number } | null | undefined
  ...
  const chapters = value.chapters as Array<{ file: string; chars: number }> | undefined
  ...
  const notes = value.notes as Array<{ kind: string; title: string }> | undefined
  if (notes !== undefined && notes.length > 0) {
    lines.push(`笔记：${notes.map(n => `${n.kind}/${n.title}`).join('、')}`)
  }
  const gate1 = value.gate1 as { status: string } | undefined
  if (gate1 !== undefined && gate1.status !== 'closed') lines.push(`门控1：${gate1.status}`)
  const gate2 = value.gate2 as { status: string; verdict?: string } | undefined
  if (gate2 !== undefined && gate2.status !== 'closed') lines.push(`门控2：${gate2.status}${gate2.verdict === undefined ? '' : `（${gate2.verdict}）`}`)
  lines.push(`下一步：${value.next ?? ''}`)
  return lines.join('\n')
}

function renderNoteResult(value: Record<string, unknown>): string {
  if (value.ok !== true) return `记录失败：${value.error ?? '未知错误'}`
  return `已记录（状态：${value.status ?? ''}）。${value.message ?? ''}`
}

function noteOutcome(job: { status: string }): string {
  return job.status === 'awaiting_gate2'
    ? '草稿已写入，门控2等待用户在面板审批。'
    : '已记录。'
}

function renderImport(value: Record<string, unknown>): string {
  if (value.ok !== true) return `导入失败：${value.error ?? '未知错误'}`
  return `已导入 OCR 结果：${value.chapters ?? 0} 个章节文件，状态 ${value.status ?? ''}。下一步：浅读建地图。`
}

function renderZlibSearch(value: Record<string, unknown>): string {
  if (value.needAuth === true) return `需要登录：${value.hint ?? ''}`
  if (value.ok !== true) return `搜索失败：${value.error ?? '未知错误'}${value.hint === undefined ? '' : `\n${value.hint}`}`
  const rows = value.rows as Array<Record<string, string>> | undefined
  if (rows === undefined || rows.length === 0) return `无结果。${value.hint ?? ''}`
  return rows.map((row, index) =>
    `${index + 1}. ${row.title} — ${row.author} (${row.year}) ${row.extension} ${row.filesize}\n   download: ${row.download}`).join('\n')
}

function renderInstall(value: Record<string, unknown>): string {
  if (value.ok !== true) {
    const gate = value.gate === true ? '【门控3】' : ''
    return `安装未执行${gate}：${value.error ?? '未知错误'}`
  }
  const result = value.result as Array<{ target: string; path: string; ok: boolean; error?: string }> | undefined
  const rows = (result ?? []).map(item =>
    `${item.ok ? '✓' : '✗'} ${INSTALL_TARGET_LABELS[item.target as InstallTarget] ?? item.target} → ${item.path}${item.error === undefined ? '' : `（${item.error}）`}`)
